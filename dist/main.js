...
const likePoke = async (name) => {
  await axios__WEBPACK_IMPORTED_MODULE_1___default().post(likeUrl, { item_id: name }).then((res) => res);
  document.getElementById(name).textContent = await getLikes(name);
};

const displayPopup = async (pokeurl) => {
  const res = await axios__WEBPACK_IMPORTED_MODULE_1___default().get(pokeurl);
...
const getLikes = async (name) => {
  let res = await axios__WEBPACK_IMPORTED_MODULE_1___default().get(likeUrl);
...
const commentSection = async (name) => {
  ...
  const res = await axios__WEBPACK_IMPORTED_MODULE_1___default().get(`${commentUrl}?item_id=${name}`);
...
const addComment = (pokeName) => {
  ...
  btn.addEventListener("click", async () => {
    await axios__WEBPACK_IMPORTED_MODULE_1___default().post(commentUrl, {
        comment: commentInput.value,
        item_id: pokeName,
        username: nameInput.value,
      })
      .then((res) => res);
    const resp = await axios__WEBPACK_IMPORTED_MODULE_1___default().get(`${commentUrl}?item_id=${pokeName}`);
